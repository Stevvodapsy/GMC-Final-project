...
import { ActivityIcon, ReportIcon, RewardIcon, ScheduleIcon } from '../common/Icons';

const ActivityFeed = () => {
  // 1. Fetch and transform data for the current user
  const userPickups = mockPickups
    ...
    .map(p => ({
      ...
      description: `Pickup ${p.status.toLowerCase()} for ${new Date(p.scheduledDate).toLocaleDateString()}`,
    }));

  const userReports = mockReports
    .filter(r => r.reportedBy === mockUser.name)
    .map(r => ({
      id: r.id,
  ...
  // 2. Combine and sort all activities chronologically
  const allActivities = [...userPickups, ...userReports, ...userRewards]
      ...
      <div className="bg-surface rounded-xl shadow-lg p-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-gray-500">A timeline of your recent actions and contributions.</p>
          </div>
        </div>
        ...
        {allActivities.length > 0 ? (
              ...
              {allActivities.map((activity) => {
                ...
                return (
                  <li key={activity.id} className="relative flex items-start gap-4">
                    <div className={`z-10 flex h-10 w-10 items-center justify-center rounded-full ${bgColor}`}>
                    ...
                    <div className="flex-1 pt-1.5">
                      <div className="flex justify-between items-center">
                        ...
                        {activity.points && (
                           <p className="font-bold text-lg text-primary">+{activity.points}</p>
                        )}
                      </div>
          ...
          <div className="text-center text-gray-500 py-8">
            ...
            <p className="text-sm mt-1">Schedule a pickup or report an issue to get started!</p>
          </div>
        )}
      </div>
    </div>
