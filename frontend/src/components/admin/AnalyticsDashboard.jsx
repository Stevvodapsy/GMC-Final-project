...
import { fetchAllRewards, fetchAnalytics } from "../../services/rewardService";
...
const AnalyticsDashboard = () => {
  ...
  useEffect(() => {
    ...
    fetchAllRewards().then(setRewards).catch(console.error);
      ...
      <div className="bg-surface p-6 rounded-xl shadow-md">
        ...
        <div className="space-y-4">
          {data.map((item) => (
            ...
            >
              <p className="col-span-1 text-sm font-medium text-gray-600 truncate">
                {item.name}
              </p>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <StatCard
          ...
          value={
            reports.filter(
              (r) =>
                r.status === Status.RESOLVED || r.status === Status.COMPLETED
            ).length
          }
          color="bg-green-100"
        />
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        <BarChart
          ...
          data={analytics.reportsByArea.map((d) => ({
            name: d.name,
            value: d.reports,
          }))}
          unit="reports"
        ...
        <BarChart
          ...
          data={analytics.recyclingActivity.map((d) => ({
            name: d.name,
            value: d.points,
          }))}
          unit="points"
